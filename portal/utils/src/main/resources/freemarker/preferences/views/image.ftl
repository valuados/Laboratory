...
<div class="controls image-selector-config single" id="image-selector-config-${preference.key}-${ns}">
    ...
    (function ($) {
        $(function () {
            $(document).ready(function () {
                var container = $('#image-selector-config-${preference.key}-${ns}');
                ...
                container.find('.save-image').on('click', function () {
                    ...
                    container.find('#input-${preference.key}-${ns}').val(url);
                    container.find('img.item-image').attr('src', url);
                ...
                container.find('.item-image').on('click', function () {
...
<style>
    .image-selector-config.single input {
        width: 300px;
        height: 31px;
        margin-left: 23px;
    }
    .image-selector-config.single .item-image {
        margin-left: -100px;
        cursor: pointer;
    }
